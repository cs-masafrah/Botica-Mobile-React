import { Minus, Plus, Trash2, AlertCircle, RefreshCw, Bug } from 'lucide-react-native';
...
import { APP_CURRENCY, convertCurrency, formatPrice, parseFormattedPrice } from '@/utils/currency';
  ...
  useEffect(() => {
    console.log('🔍 [CART SCREEN DEBUG] Current state:', {
      ...
      cartDetails: cartDetails ? {
        ...
        parsedSubTotal: parseFormattedPrice(cartDetails.formattedPrice?.subTotal || '0'),
      } : null,
    });
      ...
      <View style={styles.emptyContainer}>
        ...
        {showDebug && (
          <View style={styles.debugContainer}>
            ...
            <Text style={styles.debugText}>
              • Bagisto Cart ID: {cartDetails?.id || 'None'}
            </Text>
            <Text style={styles.debugText}>
              • Bagisto Items Count: {cartDetails?.itemsCount || 0}
            </Text>
            <Text style={styles.debugText}>
              • Bagisto Items Qty: {cartDetails?.itemsQty || 0}
            </Text>
            <Text style={styles.debugText}>
              • Bagisto Items Array: {cartDetails?.items?.length || 0} items
            </Text>
            {cartDetails?.items?.map((item, index) => (
              <Text key={index} style={styles.debugItemText}>
                &nbsp;&nbsp;{index + 1}. ID: {item.id}, Name: {item.name}, Qty: {item.quantity}
              </Text>
            ))}
            <Text style={styles.debugText}>
        ...
        <View style={styles.emptyActions}>
          <Pressable 
            ...
            disabled={manualLoading}
          >
            <RefreshCw size={18} color={Colors.white} style={manualLoading && styles.refreshIconLoading} />
          ...
          <View style={styles.debugInfo}>
            ...
            {cartDetails?.formattedPrice && (
              <>
                ...
                <Text style={styles.debugInfoText}>
                  • Parsed Subtotal: {parseFormattedPrice(cartDetails.formattedPrice.subTotal)}
                </Text>
                <Text style={styles.debugInfoText}>
                  • Formatted Subtotal: {formatPrice(cartDetails.formattedPrice.subTotal, displayCurrency)}
                </Text>
              </>
            )}
          </View>
            ...
            return (
              <View key={`${item.id}-${item.quantity}`} style={styles.cartItem}>
                {item.product.image ? (
                  ...
                  <View style={[styles.itemImage, styles.placeholderImage, isUnavailable && styles.itemImageUnavailable]}>
                  ...
                  <View style={styles.itemHeader}>
                    <View style={styles.itemInfo}>
                      ...
                      <Text style={styles.itemTotal}>
                        Total: {formatPrice(convertedUnitPrice * item.quantity, displayCurrency)}
                      </Text>
                    </View>
                    <Pressable
        ...
        {cartDetails?.formattedPrice && (
          <View style={styles.bagistoTotals}>
            <View style={styles.bagistoTotalRow}>
              <Text style={styles.bagistoTotalLabel}>Bagisto Subtotal:</Text>
              <Text style={styles.bagistoTotalValue}>
                {formatPrice(cartDetails.formattedPrice.subTotal, displayCurrency)}
              </Text>
            </View>
            <View style={styles.bagistoTotalRow}>
              <Text style={styles.bagistoTotalLabel}>Bagisto Tax:</Text>
              <Text style={styles.bagistoTotalValue}>
                {formatPrice(cartDetails.formattedPrice.taxTotal, displayCurrency)}
              </Text>
            </View>
            <View style={styles.bagistoTotalRow}>
              <Text style={styles.bagistoTotalLabel}>Bagisto Discount:</Text>
              <Text style={styles.bagistoTotalValue}>
            ...
            <View style={[styles.bagistoTotalRow, styles.bagistoGrandTotal]}>
              <Text style={styles.bagistoTotalLabel}>Bagisto Grand Total:</Text>
              <Text style={[styles.bagistoTotalValue, styles.bagistoGrandTotalValue]}>

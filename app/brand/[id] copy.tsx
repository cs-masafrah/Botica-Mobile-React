...
const BrandProductsScreen = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = useProductsByBrand(brandValue) as {
    data: { data: any[] } | null;
    isLoading: boolean;
    error: Error | null;
  };
  ...
  const renderProductItem = ({ item }: { item: any }) => {
    ...
    if (viewMode === "grid") {
      return (
        <ProductCard
          product={item}
          style={styles.gridCard} // Add specific style for grid
        />
      );
    }
    ...
    return (
      <ProductCard
        product={item}
        style={styles.listCard} // Add specific style for list
      />
    );
  };
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      /> */}
      <FlatList
        key={`products-${viewMode}`} // This forces a fresh render when viewMode changes
        data={products}
        renderItem={renderProductItem}
        keyExtractor={(item) => item.id}
        numColumns={viewMode === "grid" ? 2 : 1}
        columnWrapperStyle={
          viewMode === "grid" ? styles.productsRow : undefined
        }
        contentContainerStyle={styles.productsContainer}
        showsVerticalScrollIndicator={false}
        ListEmptyComponent={

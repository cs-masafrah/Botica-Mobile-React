...
export default function CategoryScreen() {
  ...
  // Fetch products by category (normal mode)
  const {
    data: categoryData,
    isLoading: isCategoryLoading,
    refetch: refetchCategory,
  } = useBagistoProductsByCategory(id);
  const categoryProducts = categoryData?.allProducts?.data || [];
  ...
  const minPriceValue = filterAttributesData?.getFilterAttribute.minPrice || 0;
  const maxPriceValue =
    filterAttributesData?.getFilterAttribute.maxPrice || 1000;
  ...
  // Fetch filtered products (filter mode)
  const {
  ...
  const isLoading = isCategoryLoading || (isFilterMode && isFilterLoading);
  ...
  }, [isFilterMode, filteredProductsData, categoryProducts]);
